...
let sliderContainer1, sliderContainer2, sliderContainer3, sliderContainer4, sliderContainer5;
let someBall;
function setup() {
  ...
  sliderContainer5.slider.oninput = () =>{
    if (focusedBall!=undefined){
      focusedBall.damping = focusedBall.bodyDamping = Number(sliderContainer5.slider.value);
    };
  ...
  button3.onclick = ()=>{
    ...
    setTimeout(()=>{button3.style.backgroundColor = 'tomato';},10);
  };
  ...
  for (let i=0; i<3; i++){
    ...
    someBall.lineColor = "rgba("+random(0, 255)+","+random(0, 255)+","+random(0,255)+")";
  };
};
...
function mousePressed(){
  for (ball of body.prototype.bodyArray){
    if (sqrt((mouseX-10-ball.x)**2+(mouseY-10-ball.y)**2)<ball.r){
      lockedBall = ball;
      focusedBall = ball;
...
function mouseReleased(){
  ...
  for (ball of body.prototype.bodyArray){
    if (focusedBall!=ball){ball.fillColor = "#000";};
  };
};
...
function body(renderer, radius=1, mass=10, fillColor="#000", lineWidth=3, lineColor="darkgray"){
  //Other bodies
  ...
  body.prototype.draw = (velocityArrow = true) => {
    for (BODY of body.prototype.bodyArray){
      BODY.draw();
      if (velocityArrow){BODY.displayVelocityArrow()};
    };
  };
  ...
  body.prototype.bodyCollision = () => {
    ...
    if (this.bodyArray.length<2){return null};
    ...
    for (let i=0; i<this.bodyArray.length;i++){
      for (let j=i+1; j<this.bodyArray.length; j++){
        ...
        if (distp1<b1.r+b2.r){
          ...
          b1.vx = u1x*b1.bodyDamping; b1.vy = u1y*b1.bodyDamping;
          b2.vx = u2x*b2.bodyDamping; b2.vy = u2y*b2.bodyDamping;
        };
      };
    };
  };
  ...
  this.displayVelocityArrow = () => {
    ...
    this.context.lineTo(this.vx+3*this.vy/this.speed, -3*this.vx/this.speed+this.vy);
    this.context.lineTo((this.speed+6)*this.vx/this.speed,(this.speed+6)*this.vy/this.speed);
    this.context.lineTo(this.vx-3*this.vy/this.speed, 3*this.vx/this.speed+this.vy);
    this.context.lineTo(this.vx, this.vy);
    this.context.closePath();
    this.context.stroke();
