...
function setup() {
    ...
    plotCanvas.height = plotCanvas.width = H;
    plotCanvas.getContext("2d").fillRect(0, 0, H, H);
    plotCanvas.getContext("2d").beginPath();
    plotCanvas.getContext("2d").strokeStyle = "white";
    plotCanvas.getContext("2d").lineWidth = 4;
    plotCanvas.getContext("2d").rect(0, 0, H, H);
    plotCanvas.getContext("2d").closePath();
    plotCanvas.getContext("2d").stroke();
    plotCanvas.style.height = plotCanvas.style.width = String(H) + "px";
    ...
    body.prototype.ekPlot = new makePlot(plotCanvas);
    body.prototype.ekPlot.build([H / 20, H / 20, 19 * H / 20, 19 * H / 20]);
};
    ...
    if (running) {
        for (let i = 0; i <= speed; i++) {
            ...
            if (i3r1checkBoxCont.checkbox.checked) {
                body.prototype.plotEk(body.prototype.ekPlot);
            };
        };
    } else {
        for (b of body.prototype.bodyArray) {
...
function body(renderer, radius = 1, mass = 10, fillColor = "yellow", lineWidth =
    ...
    body.prototype.plotEk = (plot) => {
        ...
        plot.setAxLimits([t - 5, t + 5], [0, ekMax + 20]);
        let axesC = plot.axes.context;
        axesC.lineWidth = .2;
        for (BODY of body.prototype.bodyArray) {
            axesC.strokeStyle = BODY.fillColor;
            axesC.beginPath();
            plot.startLine([t - dt, BODY.lastEk]);
            plot.addLineTo([t, BODY.ek]);
            axesC.closePath();
        };
        plot.display();
    };
...
function makePlot(canvas) {
    ...
    this.display = () => {
        ...
        if (this.canvas.elt == undefined) {
            this.canvas.getContext('2d').setTransform(scale / 2, 0, 0, scale / 2, 0, 0);
        } else {
            this.canvas.elt.getContext('2d').setTransform(scale / 2, 0, 0, scale / 2, 0, 0);
        };
        figure.background(0);
        this.figure.context.drawImage(
            axes.elt,
        ...
        this.canvas.getContext("2d").drawImage(this.figure.elt,
